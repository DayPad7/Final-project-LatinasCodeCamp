...
const Stocks = (props) => {
  ...
  const ComprarStocks = (e) => {
    if (
      stockTotalPrice <= props.initialCash &&
      inputStocks <= props.stocks.amount
    ) {
      //To do:
      ...
      props.setStocks({
        ...props.stocks,
        amount: props.stocks.amount - inputStocks,
        usershare: props.stocks.usershare + inputStocks,
      });
    ...
    if (stockTotalPrice > props.initialCash) {
      alert(
        "No tienes suficiente dinero para realizar esta transacción Por favor revisa tu Estado de Cuenta"
      );
      setTriggerPopUp(false);
    }
  };
  ...
  const Stocksell = (e) => {
    if (inputStockSell <= props.stocks.usershare) {
      props.setStocks({
        ...props.stocks,
        amount: props.stocks.amount + inputStockSell,
        usershare: props.stocks.usershare - inputStockSell,
      });
      props.setCash(props.initialCash + stockTotalPrice);
    ...
    if (inputStockSell > props.stocks.usershare) {
      alert(
        "No se logró realizar la transacción debido a que no tienes suficientes acciones."
      );
      setTriggerPopUpSell(false);
    }
  };
    ...
    <>
      <Popup trigger={triggerPopUp}>
        <section className="text-xl  text-center">
          ...
          <p>
            {" "}
            Te recordamos que cada accion cuesta: {props.stocks.price} y el
            total seria de USD {stockTotalPrice}{" "}
          </p>

          <button
            onClick={ComprarStocks}
      ...
      <Popup trigger={triggerPopUpSell}>
        <section className="text-xl  text-center">
          ...
          <p>
            {" "}
            Te recordamos que cada accion cuesta: USD {props.stocks.price} y el
            total a recibir seria de USD {stockTotalPrice}{" "}
          </p>
          <button
            onClick={Stocksell}
            className=" w-40  text-xl rounded-md py-2 m-1 bg-red-300 hover:bg-red-400 ..."
      ...
      <section className=" flex flex-col text-center h-full w-full m-10 ">
        ...
        <h4 className="text-1xl font-semibold text-center text-black">
          {" "}
          Estas son tus acciones: {props.stocks.usershare}{" "}
        </h4>
        ...
        <section className=" flex flex-col text-center h-full w-full">
          <form onSubmit={handleSubmitStocks}>
            ...
            <input
              value={inputStocks}
              onChange={(e) => SetinputStocks(parseInt(e.target.value))}
              type="number"
          ...
          <form onSubmit={handleSubmitStockSell}>
            ...
            <br />
            <input
              value={inputStockSell}
              onChange={(e) => SetinputSell(parseInt(e.target.value))}
              type="number"
              className="mt-1 px-3 py-2 bg-white shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 rounded-md sm:text-sm focus:ring-1 "
              placeholder="Cantidad de acciones"
            />
